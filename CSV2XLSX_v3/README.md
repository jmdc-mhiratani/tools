...
[![Release](https://img.shields.io/badge/release-v2.1.0-green.svg)](https://github.com/your-username/CSV2XLSX_v2/releases)
...
このプロジェクトでは、Python仮想環境を使用した開発を行っています。
パッケージマネージャーとして[uv](https://github.com/astral-sh/uv)を採用し、高速な依存関係管理を実現しています。
...
# 2. プロジェクトディレクトリに移動
cd G:\works\apps\CSV2XLSX_v2

# 3. 依存関係をインストール（仮想環境を自動作成）
uv sync
...
### 2025年1月25日
- 🔧 Python 3.13.7 + uv仮想環境構築完了
- 📝 開発ロードマップ作成（6フェーズ計画）
- 🎨 SimpleMainWindowによる新UI実装
- 📚 開発環境ドキュメント整備
- ✅ テスト環境構築（pytest, black, ruff）

詳細は[開発ロードマップ](docs/development_roadmap.md)と[開発環境](docs/development_environment.md)を参照してください。
