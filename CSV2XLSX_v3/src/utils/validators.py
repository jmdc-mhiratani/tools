...
class DataValidator:
        ...
        try:
            ...
            if confidence < 0.7:
                result["warnings"].append(
                    f"エンコーディング検出の信頼度が低いです: {confidence:.2f}"
                )
            ...
            try:
                ...
                # データ読み込み
                df = pd.read_csv(
                    file_path, encoding=encoding, sep=delimiter, nrows=1000
                )
                ...
                if inconsistent_types:
                    result["warnings"].append(
                        f"データ型が不整合な列があります: {inconsistent_types}"
                    )
    ...
    def _validate_header(df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            return not len(numeric_headers) > len(df.columns) * 0.5
    ...
    def validate_excel_file(file_path: Path) -> dict[str, Any]:
        ...
        result: dict[str, Any] = {
            ...
            "errors": [],
            "warnings": [],
            "info": {"sheets": [], "total_rows": 0, "total_columns": 0},
        }
        ...
        try:
            ...
            for sheet_name in excel_file.sheet_names:
                try:
                    ...
                    result["warnings"].append(
                        f"シート'{sheet_name}'の読み込みに失敗: {str(e)}"
                    )
...
class SecurityValidator:
    ...
    def validate_file_security(cls, file_path: Path) -> dict[str, Any]:
        ...
        Returns:
            検証結果
        """
        result: dict[str, Any] = {"is_safe": True, "risks": [], "warnings": []}
        ...
        try:
            ...
            if available_gb < 1:
                ...
                result["recommendations"].append(
                    "メモリ不足：他のアプリケーションを終了してください"
                )
            elif available_gb < 2:
                result["recommendations"].append(
                    "メモリが少なめです：大容量ファイル処理時は注意"
                )
...
class ConversionValidator:
    ...
    def validate_conversion_request(
        input_files: list[Path], output_format: str, output_directory: Path
    ) -> dict[str, Any]:
        """
        変換リクエストの検証
        ...
        if result["file_conflicts"]:
            result["warnings"].append(
                f"{len(result['file_conflicts'])}個のファイルが上書きされます"
            )
